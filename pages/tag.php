...
// Delete
	...
	$tag = new Tag($tag_id, rex_config::get("d2u_references", "default_lang"));
	$tag->delete();
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$tag = new Tag($entry_id, rex_config::get("d2u_references", "default_lang"));
							$readonly = TRUE;
...
//							d2u_addon_backend_helper::form_mediafield('d2u_references_picture', '1', $tag->picture, $readonly);
							$options_tags = [];
							foreach (Reference::getAll(rex_config::get("d2u_references", "default_lang")) as $reference) {
...
if ($func == '') {
	$query = 'SELECT tags.tag_id, name '
		...
		. 'LEFT JOIN '. rex::getTablePrefix() .'d2u_references_tags_lang AS lang '
			. 'ON tags.tag_id = lang.tag_id AND lang.clang_id = '. rex_config::get("d2u_references", "default_lang") .' '
		.'ORDER BY name ASC';
