 ...
 * Tag.
 */
class Tag implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    public int $tag_id = 0;
    ...
    {
        if ($this->tag_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->tag_id, $sourceClangId);
        if ('' === $source->name) {
            return false;
        }
